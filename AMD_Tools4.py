...
def map_figs(data, lat, lon, label, cmapstr, minmax, filename, overlay, legend, asp=None):
    ...
    # [修正] plt.close()前にnormを保存。閉じたfigのcsを別figのcolorbarに渡すと
    # UserWarning が出るため、ScalarMappableを使って新figに依存しないオブジェクトを作成する。
    cs_norm = cs.norm
    plt.close()
    ...
    if data.dtype == np.dtype('<M8[D]'):
        ...
        cmap.set_bad('w', 1.0)
        # cb = fig.colorbar(cs, cax=ax, format=DateFormatter('%b %d'))  # 修正前
        sm = cm.ScalarMappable(cmap=cmap, norm=cs_norm)
        sm.set_array([])
        cb = fig.colorbar(sm, cax=ax, format=DateFormatter('%b %d'))
    else:
        # cb = fig.colorbar(cs, cax=ax)  # 修正前
        sm = cm.ScalarMappable(cmap=cmap, norm=cs_norm)
        sm.set_array([])
