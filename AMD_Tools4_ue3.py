# -*- coding: utf-8 -*-
"""
uehara一部、修正した関数を追加  AMD_Tools4.py本体を最新版20250901に変更し、linefig_ueを追加したAMD_Tools4_ue2.pyとした。
20260616 AMD_Tools4_ue2.py から AMD_Tools4_ue3.py へリネーム。Basic認証 → Oracle OAuthデバイスフロー認証に移行。
...
AMD_Tools4.py
   ...
   21. GetMetDataHourlyX:メッシュ農業気象データ（時別値）を取得する関数(xarray出力版)。
    ...
    改変履歴：
    20250901 XarrayのFutureWarningに対応
    20250806 関数linefigの機能強化
...
"""
# [修正前] Basic認証（ID+パスワード直書き）
#USER="uehr"
#PASSWORDS=["kfa6NK6o2024", "1JCHzyV92025"] #20250109 , 20260109
##PASSWORDS=["1JCHzyV92025", "パスワード"] #データ利用更新　20260109から。
...
import copy
import xarray as xr
import matplotlib.pyplot as plt #実況部分の日付ラベルの色付け用に追加　上原20250605
import datetime #実況部分の日付ラベルの色付け用に追加　上原20250605
import matplotlib.dates as mdates  #実況部分の日付ラベルの色付け用に追加　上原20250605
...
plt.rcParams['font.family'] = 'Hiragino Sans'  # macOS標準フォント（游明朝体のダウンロード要求を回避） 上原20260410
import pandas as pd

# [修正後] Oracle OAuthデバイスフロー認証定数（import後に配置）
...
#上原追加関数
def linefig_ue(time, var, title='', ylabel='', llabel='',
               timeref=None, ref=None, ylabelref='', llabelref='',
               commony=True, figsize=(12, 4), filename=None,
               ma_window=None, ma_window_ref=None,
               show_label=True, xtick_interval=2,
               ylim=None, ylim_ref=None):
    """
    概要：主線・参照線に移動平均・数値ラベル表示を追加した折れ線グラフ描画関数
        ・主線は太青・丸マーカ付き
        ・参照線(細赤線)を追加できる
        ・横軸は主線/参照線で共通にも独立にも設定可能
        ・グラフタイトル、軸ラベル、データ凡例付き
        ・日本時間8時を基準に過去データの色を青に変更
        ・横軸ラベル形式は MM/DD に固定
        ・ラベルの表示間隔と縦軸の範囲を引数で調整可能

    引数(追加)：
        ma_window: 主線に対する移動平均の窓サイズ（例：7なら7点平均）
        ma_window_ref: 参照線に対する移動平均の窓サイズ
        show_label: 数値ラベルのon/off
        xtick_interval: 横軸ラベル間隔（日）。0またはNoneなら自動
        ylim: 主線のy軸範囲（例：(0, 50)）
        ylim_ref: 参照線（副軸）のy軸範囲（例：(0, 100)）
    """
    ...
    threshold_date = (now_jst - datetime.timedelta(days=2)).date() if now_jst.hour < 8 else (now_jst - datetime.timedelta(days=1)).date()

    # 主線：移動平均処理
    if ma_window is not None and ma_window > 1:
        var_plot = np.convolve(var, np.ones(ma_window) / ma_window, mode='valid')
        time_plot = time[(ma_window - 1) // 2: -(ma_window // 2)] if len(time) > ma_window else time
    else:
        var_plot = var
        time_plot = time
    ...
    # 折れ線（主線）
    ax.plot(time_plot, var_plot, label=llabel, linewidth=2, color='blue')

    # データ点とラベル（主線）
    for x, y in zip(time_plot, var_plot):
        color = 'blue' if np.datetime64(x, 'D') <= np.datetime64(threshold_date) else 'black'
        ax.plot(x, y, marker="o", color=color, markersize=6)
        if show_label:
            ax.annotate(f"{y:.1f}", xy=(x, y), xytext=(15, 0),
    ...
    # 参照線処理
    if ref is not None:
        if ma_window_ref is not None and ma_window_ref > 1:
            ref_plot = np.convolve(ref, np.ones(ma_window_ref) / ma_window_ref, mode='valid')
            if timeref is None:
                timeref_plot = time[(ma_window_ref - 1) // 2: -(ma_window_ref // 2)] if len(time) > ma_window_ref else time
            else:
                timeref_plot = timeref[(ma_window_ref - 1) // 2: -(ma_window_ref // 2)] if len(timeref) > ma_window_ref else timeref
        else:
            ref_plot = ref
            timeref_plot = timeref if timeref is not None else time

        ax_ref = ax if commony else ax.twinx()
        if not commony:
            ax_ref.set_ylabel(ylabelref)
            if ylim_ref is not None:
                ax_ref.set_ylim(ylim_ref)

        ax_ref.plot(timeref_plot, ref_plot, label=llabelref, color='brown', linewidth=1.5, linestyle='--')

#        for x, y in zip(timeref_plot, ref_plot):
#            color = 'blue' if np.datetime64(x, 'D') <= np.datetime64(threshold_date) else 'brown'
#            ax_ref.plot(x, y, marker="x", color=color)
            #参照線（平年値）にはグラフ内の数値ラベルは表示しない
#            if show_label:
#                ax_ref.annotate(f"{y:.1f}", xy=(x, y), xytext=(0, -10),
#                                textcoords="offset points", ha='center', fontsize=9, color=color)

        if llabelref != '':
            ax_ref.legend(loc='upper left' if commony else 'upper right')
    ...
    for label in ax.get_xticklabels():
        try:
            label_date = datetime.datetime.strptime(label.get_text(), '%m/%d').replace(year=now_jst.year).date()
            if label_date <= threshold_date:
                label.set_color('blue')
        except ValueError:
            continue

    # 左右余白を縮小（デフォルト left=0.125, right=0.9 の約1/5）
...
# [修正後] Oracle OAuthデバイスフロー認証関数群（AMD_Tools4.py 20260216版より移植）
...
def url2dh(url):
    url = url.replace("\\","/")
    a,b = url.split("?")
    no_pw = not url.startswith("https://")  # [修正] 旧: amd.rd.naro.go.jp等を個別判定 → https://全般に統一
    ...
    if no_pw:
        ...
        #    ...
        # [修正後] Bearer Token認証（Oracle OAuthトークン）
...
def GetMetData_Area(element, timedomain, lalodomain, area=None,
               cli=False, namuni=False, url='https://amd2.rd.naro.go.jp/opendap-api/AMD/'):
    """
概要：
    メッシュ農業気象データを、気象データをデータ配信サーバーまたはローカルファイルから取得する関数(Area区切り対応版)。
書式：
　GetMetData_Area(element, timedomain, lalodomain, area=None, cli=False, namuni=False, url='https://amd2.rd.naro.go.jp/opendap-api/AMD/')
引数(必須)：
...
def GetSceData_Area(element, timedomain, lalodomain, model, scenam, area=None,
               namuni=False, url='https://amd2.rd.naro.go.jp/opendap-api/AMS'):
    """
概要：
    気候予測シナリオデータを、データ配信サーバーまたはローカルファイルから取得する関数(Area区切り対応版)。
書式：
　GetSceData_Area(element, timedomain, lalodomain, model, scenam, area=None, namuni=False, url='https://amd2.rd.naro.go.jp/opendap-api/AMS')
引数(必須)：
    element：気象要素記号で、'TMP_mea'などの文字列で与える
    timedomain：取得するデータの時間範囲で、['2008-05-05', '2008-05-05']
                のような文字列の2要素リストで与える。特定の日のデータを
...
def GetGeoData_Area(element, lalodomain, area=None,
               namuni=False, url='https://amd2.rd.naro.go.jp/opendap-api/AMD/'):
    """
概要：
    土地利用区分等の地理情報をデータ配信サーバーまたはローカルファイルから取得する関数(Area区切り対応版)。
書式：
    GetGeoData_Area(element, lalodomain, area=None, namuni=False, url='https://amd2.rd.naro.go.jp/opendap-api/AMD/')
引数(必須)：
...
def GetGeoData(element, lalodomain, namuni=False,
               url='https://amd2.rd.naro.go.jp/opendap-api/AMD/'):
    """
概要：
    土地利用区分等の地理情報をデータ配信サーバーまたはローカルファイルから取得する関数(1次メッシュ区切り対応)。
書式：
    GetGeoData(element, lalodomain, namuni=False, url='https://amd2.rd.naro.go.jp/opendap-api/AMD/')
引数(必須)：
    element：地理情報記号で、'altitude'などの文字列で与える
    lalodomain：取得するデータの緯度と経度の範囲で、
        [36.0, 40.0, 130.0, 135.0] のように緯度,緯度,経度,経度の順で指定する。
...
書式：
    GetGeoDataX(element, lalodomain, url='https://amd2.rd.naro.go.jp/opendap-api/AMD/')
引数(必須)：
    element：地理情報記号で、'altitude'などの文字列で与える
    lalodomain：取得するデータの緯度と経度の範囲で、
        [36.0, 40.0, 130.0, 135.0] のように緯度,緯度,経度,経度の順で指定する。
...
書式：
　GetMetData(element, timedomain, lalodomain, cli=False, namuni=False, url='https://amd2.rd.naro.go.jp/opendap-api/AMD/')
引数(必須)：
    element：気象要素記号で、'TMP_mea'などの文字列で与える
    timedomain：取得するデータの時間範囲で、['2008-05-05', '2008-05-05']
                のような文字列の2要素リストで与える。特定の日のデータを
...
def GetMetDataX(element, timedomain, lalodomain,
                 cli=False, url='https://amd2.rd.naro.go.jp/opendap-api/AMD/'):
    """
概要：
    メッシュ農業気象データを、気象データをデータ配信サーバーまたはローカルファイルから取得する関数(1次メッシュ区切り対応, xarray出力版)。
書式：
　GetMetDataX(element, timedomain, lalodomain, cli=False, url='https://amd2.rd.naro.go.jp/opendap-api/AMD/')
引数(必須)：
    element：気象要素記号で、'TMP_mea'などの文字列で与える
    timedomain：取得するデータの時間範囲で、['2008-05-05', '2008-05-05']
                のような文字列の2要素リストで与える。特定の日のデータを
...
def GetSceData(element, timedomain, lalodomain, model, scenam, namuni=False, url='https://amd2.rd.naro.go.jp/opendap-api/AMS'):
    """
概要：
    気候予測シナリオデータを、気象データをデータ配信サーバーまたはローカルファイルから取得する関数(1次メッシュ区切り対応版)。
書式：
　GetSceData(element, timedomain, lalodomain, model, scenam, namuni=False, url='https://amd2.rd.naro.go.jp/opendap-api/AMS')
引数(必須)：
    element：気象要素記号で、'TMP_mea'などの文字列で与える
    timedomain：取得するデータの時間範囲で、['2008-05-05', '2008-05-05']
                のような文字列の2要素リストで与える。特定の日のデータを
...
def GetSceDataX(element, timedomain, lalodomain, model, scenam,
               url='https://amd2.rd.naro.go.jp/opendap-api/AMS'):
    """
概要：
    気候予測シナリオデータを、気象データをデータ配信サーバーまたはローカルファイルから取得する関数(1次メッシュ区切り対応版, xarray出力版)。
書式：
　GetSceDataX(element, timedomain, lalodomain, model, scenam, url='https://amd2.rd.naro.go.jp/opendap-api/AMS')
引数(必須)：
    element：気象要素記号で、'TMP_mea'などの文字列で与える
    timedomain：取得するデータの時間範囲で、['2008-05-05', '2008-05-05']
                のような文字列の2要素リストで与える。特定の日のデータを
...
def GetMetDataHourly(element, timedomain, lalodomain,
                cli=False, namuni=False, url='https://amd2.rd.naro.go.jp/opendap-api/AMD_Hourly'):
    """
概要：
    メッシュ農業気象データ時別値を、気象データをデータ配信サーバーまたはローカルファイルから取得する関数。
書式：
　GetMetDataHourly(element, timedomain, lalodomain, namuni=False, url='https://amd2.rd.naro.go.jp/opendap-api/AMD_Hourly')
引数(必須)：
...
def GetMetDataHourlyX(element, timedomain, lalodomain,
                cli=False, url='https://amd2.rd.naro.go.jp/opendap-api/AMD_Hourly'):
    """
概要：
    メッシュ農業気象データ時別値を、気象データをデータ配信サーバーまたはローカルファイルから取得する関数(xarray出力版)。
書式：
　GetMetDataHourlyX(element, timedomain, lalodomain, url='https://amd2.rd.naro.go.jp/opendap-api/AMD_Hourly')
引数(必須)：
    element：気象要素記号で、'TMP'などの文字列で与える。（気温: 'TMP', 相対湿度: 'RH', 下向き長波放射量: 'DLR'）
    timedomain：取得するデータの時間範囲で、['2008-05-05T13:00', '2008-05-05']
                のような文字列の2要素リストで与える。日付と時刻の間は'T'で区切る。
...
def main():
    ...
    parser.add_argument('-v', '--version', action='version', version='%(prog)s 4.0.11')
